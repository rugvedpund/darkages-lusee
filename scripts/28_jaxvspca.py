...
def doPCA(sim):
    ...
    proj = simpca.get_pca_proj(fg, da, cmb, delta_pca, "times", "freqs", other_dims=[])
...
wda.loss.plot(label="da")
wcmb.loss.plot(label="cmb")
plt.yscale("log")
plt.legend()
plt.show()
...
wda.wnorm.plot(label="da")
wcmb.wnorm.plot(label="cmb")
plt.legend()
plt.show()

# %%

wda.woptim.plot(label="da")
wcmb.woptim.plot(label="cmb")
plt.legend()
plt.show()
